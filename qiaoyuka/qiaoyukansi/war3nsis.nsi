...
; 安装程序初始定义常量
!define PRODUCT_NAME "巧遇卡自动聊天"
!define PRODUCT_VERSION "1.0"
!define PRODUCT_PUBLISHER "pipi"
!define PRODUCT_WEB_SITE "http://pipi.1kapp.com/?p=45"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\qiaoyuka.exe"
...
; 安装完成页面
!define MUI_FINISHPAGE_RUN "$INSTDIR\qiaoyuka.exe"
...
Section -Post
  ...
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\qiaoyuka.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\qiaoyuka.exe"
